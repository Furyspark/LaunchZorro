...
Categories.iconPath = function() {
  return Core.baseData.baseDir + "/icons/categories/";
};
...
Profiles.add = function(name) {
  ...
  var iconPath = this.iconPath(dirName.toLowerCase()) + name.toLowerCase() + ".png";
  fs.access(iconPath, fs.constants.F_OK, function(err) {
    var iconElem = document.createElement("img");
    if(!err) iconElem.src = iconPath;
    else iconElem.src = Profiles.iconPath("generic") + "blank.png";
    iconElem.width = "32";
    iconElem.height = "32";
    elem.insertBefore(iconElem, txtElem);
  });
